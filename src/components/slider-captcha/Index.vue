...
import { getSliderAPI, validateSliderAPI } from "@/apis/api/captcha"
...
//获取验证码图片
const getCaptcha = () => {
    getSliderAPI().then((res) => {
        if (res.data.code === statusCode.success) {
            state.top = res.data.data.y;
            state.bg = `data:image/jpg;base64,${res.data.data.bg_img}`;
            state.subImg = `data:image/jpg;base64,${res.data.data.slider_img}`;
        }
    })
}
...
const moveEnd = () => {
    state.canMove = false;
    setMsg("wait");
    validateSliderAPI(Math.round(state.left)).then((res) => {
        if (res.data.code === statusCode.success && res.data.data.result === true) {
            setMsg("success");
            setTimeout(() => {
                close();
            }, 500)
